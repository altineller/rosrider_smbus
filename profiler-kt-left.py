...
def read_status_packet(bus):
    ...
    cs_left_amps = cs_left_raw * CURRENT_CONVERSION_UNSCALER
    
    rpm_left_raw = status[20] << 8 | status[21]
    enc_dir_left = (status[24] & 0x0F) - 1
    rpm_left_signed = rpm_left_raw * ROUNDS_PER_MINUTE_FACTOR * enc_dir_left
    
    pwm_left_read = unsignedToSigned(status[16] << 8 | status[17], 2)
    
    # --- TRUE DC MOTOR CURRENT CALCULATION ---
    i_true_dc_motor = max(0.0, bus_current_total_amps - MCU_OFFSET_CURRENT_A) 
    
    return {
        'pwm_command_read': pwm_left_read,
        'rpm_left': rpm_left_signed,
        'bus_voltage': bus_voltage_volts,
        'cs_left_amps': cs_left_amps,
...
with SMBus(BUS_NUM) as bus:
    ...
    with open(LOG_FILENAME, 'w') as f:
        ...
        f.write('PWM_Command_Sent,RPM_Output,Bus_Voltage_V,CS_Left_A,I_Bus_Motor_A,V_App_V,Kt_SI_Nm_A,Kt_ADC_Nm_A\n')
        ...
        for pwm_cmd in range(PWM_START, PWM_END + PWM_STEP, PWM_STEP):
            ...
            for i in range(num_commands):
                send_pwm_command(bus, pwm_cmd, 0)
            ...
            for _ in range(AVERAGE_COUNT):
                ...
                if data and error is None:
                    sum_rpm += data['rpm_left']
                    sum_voltage += data['bus_voltage']
                    sum_cs_left += data['cs_left_amps']
            ...
            if valid_reads > 0:
                ...
                avg_cs_left = sum_cs_left / valid_reads
                ...
                if abs(omega_motor) > 1.0: # Avoid noise/stiction
                    ...
                    v_ir_adc = avg_cs_left * MOTOR_RESISTANCE_OHM
                    bemf_adc = v_app - v_ir_adc
                    kt_adc = bemf_adc / omega_motor

                # Log to console and file
                f.write(f"{pwm_cmd},{avg_rpm_output:.4f},{avg_voltage:.4f},{avg_cs_left:.4f},{avg_i_true_dc_motor:.4f},{v_app:.4f},{kt_si:.6f},{kt_adc:.6f}\n")
                print(f"{pwm_cmd:3d} | {avg_rpm_output:5.1f} | {avg_i_true_dc_motor:5.3f} | {avg_cs_left:5.3f} | {kt_si:.4f} | {kt_adc:.4f}")
